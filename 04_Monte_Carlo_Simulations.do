...
// Create two independent Random Walks (drunkards wandering unrelated paths)
// Draw errors from standard normals
gen e1 = rnormal()
gen e2 = rnormal()
// Cumulative sum (sum) of random shocks 
gen y1 = sum(e1)     
gen y2 = sum(e2)
...
twoway (tsline y1) (tsline y2, yaxis(2)), ///
    graphregion(color(white)) legend(position(6) col(2) region(lstyle(none))) ///
	ytitle("Level") ///
    title("The Visual: Spurious Correlation Trap") ///
    note("These variables are completely unrelated.") 

// 4. The "Lie" (Regression in Levels)
reg y1 y2
// Look at the t-statistic > 1.96. The model says "Significant". This is spurious.

// -----------------------------    
//  Saving Outputs Post Commands
// -----------------------------    

// Here are two ways to save p-values/ output in simulations
reg y1 y2

// How to Plunder the Stores 
help reg
ereturn list        // <--- Shows e(N), e(r2), e(b), etc.
scalar r2a = e(r2_a)
display "R^2-adj: " r2a 
matrix list e(b)
...
display _se[y2]                     // s.e of coefficient
...
// ----------------------------- 
// We need to do this 10000 or 5000 times to prove the "Lie" happens systematically.
...
program mysimprogram, rclass        // create a program 
    ...
    // We pass arguments so we can change parameters later without rewriting code
    args vv yy bb 
